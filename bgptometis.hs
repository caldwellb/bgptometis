...
outputFile asnMapFinal conMapFinal month year = do
    let n            = length . Map.keys  $ conMapFinal
        ...
        asnMapLst    = Map.toAscList asnMapFinal
    ...
    pb <- newProgressBar defStyle 10 (Progress 0 (length asnMapLst) ())
    forM_ asnMapLst $ \(asnNum, asnweights) -> do
        appendFile outMetis (show (retrieveIPWeight asnNum asnMapFinal) ++ " " ++ formatASConnections (conMapFinal Map.! asnNum) ++ "\n")
