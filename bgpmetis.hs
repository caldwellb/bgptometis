...
main = do
    ...
    case args of
        (month:year:xs) -> do
            ...
            let acceptedFiles = [head . filter isBZ2File $ allFilePaths]
            putStrLn "Converting .bz2 inputs into maps"
            (rawASNMap, conMap) <- (\(a,b) -> (unionsASNData a, unionsASConnections b)) . unzip <$> forM acceptedFiles handleSingleFile
